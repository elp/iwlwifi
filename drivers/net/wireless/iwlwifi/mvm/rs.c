...
static s32 expected_tpt_siso_20MHz[4][IWL_RATE_COUNT] = {
	...
	{0, 0, 0, 0, 49, 0,  97, 145, 192, 285, 275, 420, 464, 551, 0},
...
static s32 expected_tpt_mimo2_80MHz[4][IWL_RATE_COUNT] = {
	...
	{0, 0, 0, 0, 424, 0, 920, 1338, 1732, 2464, 3116, 3418, 3705, 4225, 4545},
};
			 ...
			 struct sk_buff *skb)
{
	...
	if ((mac_index < 0) ||
	    ...
	    (rs_index != mac_index)) {
		IWL_DEBUG_RATE(mvm,
			       "initial rate %d does not match %d (0x%x) bw 0x%x mac bw 0x%x\n",
				...
				int index)
{
	...
	while (1) {
		...
		case IWL_LEGACY_SWITCH_ANTENNA:
			...
			if (WARN_ON_ONCE(tx_chains_num <= 1))
				break;
				 ...
				 struct ieee80211_sta *sta, int index)
{
	...
	case IWL_BT_COEX_TRAFFIC_LOAD_CONTINUOUS:
		/* A - avoid antenna B and MIMO, switch GI. B - switch to A */
			     ...
			     struct iwl_lq_sta *lq_sta, u32 new_rate)
{
	...
	while (index < LINK_QUAL_MAX_RETRY_NUM) {
		 ...
		 * rs_get_lower_rate() will change type to LQ_LEGACY_A or LQ_LEGACY_G. */
		use_ht_possible = 0;
...
#ifdef CONFIG_MAC80211_DEBUGFS
static void rs_dbgfs_set_mcs(struct iwl_lq_sta *lq_sta, u32 *rate_n_flags)
{
	struct iwl_mvm *mvm;
	u8 valid_tx_ant;
	u8 ant_sel_tx;
