			 ...
			 struct ieee80211_sta_ht_cap *sta_ht_inf)
{
	...
	if ((cpu_to_le32(iwl_ht_conf->ht_protection << RXON_FLG_HT_OPERATING_MODE_POS)
	     & RXON_FLG_HT_PROT_MSK) &&
	    (iwl_ht_conf->extension_chan_offset == IEEE80211_HT_PARAM_CHA_SEC_NONE))
		return 0;

	/* do not check for IEEE80211_HT_CAP_SUP_WIDTH_20_40
	 * the bit will not set if its pure 40MHz case
	 */
	if (sta_ht_inf) {
